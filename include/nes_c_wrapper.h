...
#ifndef NES_C_WRAPPER_H_
#define NES_C_WRAPPER_H_
...
static std::string error_names[] = {ERROR_LIST};

#undef X

#define X(error, message) message

static std::string error_messages[] = {ERROR_LIST};
...
static void nes_memory_init(const char *rom_filename, ppu_s *ppu) {
  int err;
  char e_context[LEN_E_CONTEXT];
  *e_context = '\0';
  if ((err = memory_init(rom_filename, ppu, e_context)) < 0) {
    throw NESError((error_e)-err, e_context);
  }
}

static void nes_ppu_init(ppu_s **ppu, void (*put_pixel)(int, int, uint8_t)) {
  int err;
...
static void nes_cpu_init(cpu_s **cpu, int nestest) {
  int err;
...
static void nes_cpu_exec(cpu_s *cpu) {
  char e_context[LEN_E_CONTEXT];
  *e_context = '\0';
  int exec_status = 0;
